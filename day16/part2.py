...
def track_light(start_point: tuple[tuple[int, int], int]) -> int:
    """
...
for i in range(len(grid)):
    energized = track_light(((i, 0), WEST))
    max_energized = max(max_energized, energized)
    energized = track_light(((i, MAX_COL), EAST))
    max_energized = max(max_energized, energized)

for i in range(len(grid[0][:-1])):
    energized = track_light(((0, i), SOUTH))
    max_energized = max(max_energized, energized)
    energized = track_light(((MAX_ROW, i), NORTH))
    max_energized = max(max_energized, energized)
