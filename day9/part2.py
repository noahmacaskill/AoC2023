...
def extrapolate_value(values: List[int]) -> int:
    """
    Extrapolates the preceding value in a list

    Parameters:
    values (List[int]): List of integers

    Returns:
    int: The extrapolated preceding value in the sequence
    """
    differences = []
    ...
    while len(differences) > 1:
        ...
        first_element = differences.pop()
        extrapolated_element = first_element - difference
